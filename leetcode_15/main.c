...
int** threeSum(int* nums, int numsSize, int* returnSize, int** returnColumnSizes){
    ...
    *returnSize = 0;
    ...
    // LeetCode 用realloc报错
    int** returnArray = (int**)malloc(sizeof(int*) * (numsSize)*10);
    *returnColumnSizes = (int*)malloc(sizeof(int) * (numsSize)*10);
    // 调用stdlib库的快速排序函数
    qsort(nums, numsSize, sizeof(int), comp);
    
    // 第一层循环用来遍历cur，第二层循环用来双指针往中间夹
    while((nums[cur] <= 0) && (cur < numsSize - 2)) // 当相反数大于0时停下，后面的都是正数了，肯定不可能
    {
        // 每次更新
        low = cur + 1;
        high = numsSize - 1;
